...
| **DigiLocker** (simulated national document wallet) | Document verification with citizen consent | OAuth 2.0 authorization code → bearer, REST / JSON envelope | `client_id`/`client_secret` at the token endpoint; citizen consent on the locker's own screen | `digilocker-v1`: `uri`, `doctype`, `issuer`, `holder`, `fields`, `hash`, `signature` | issuer HMAC signature recomputed → `documents.verified`; `holder.name/dob` compared with the registry, **transient**; `uri` kept as reference only — document content is never stored |
...
| 1:00 | **Citizen submits once** | Window B → **Start a service** → choose a scheme card (e.g. *Drip irrigation subsidy*) → pick the crop/course and district → **Connect DigiLocker** (you land on the *simulated* DigiLocker consent screen — a different site with a yellow SIMULATED banner — press **Allow**) → tick both consent boxes → **Submit application** | Note the three consent receipts (DigiLocker, eligibility, treasury) and the single `APP-MH-2026-…` ID. Copy the **TXN** ID. |
| 1:45 | **Watch it propagate, then pause for a human** | Window B stays on the application page | Within ~5 s stages 1–3 turn *Completed* (identity, **document verification** — issuer signature and holder match per document — and eligibility) and stage 4 reads **Awaiting officer decision** — the citizen sees *"Your application is with the department for a decision"*. "The fabric never sanctions a benefit on its own." |
| 2:00 | **Missing document (Rohan)** | Window D → sign in as `rohan@demo.in` → open the *Post-matric scholarship* marked **Documents required** → **Share from DigiLocker** → on the simulated locker the *Income certificate* reads *Not in your locker* → **Get from Revenue Department (simulated)** → **Allow** | The workflow was paused at *Document verification* with a precise hold (`INCER` missing) instead of failing. After sharing, verification resumes on its own; the officer (`sjsa@demo.in`) then sees all three documents with *Issuer signature valid · Holder matches registry* and can **Preview** a masked copy (XXXX ID, XXXX-XX-DD birth date, issuer seal, hash). Nothing is stored by Sampark — the preview is fetched live under the citizen's consent. |
| 2:30 | **Officer sanctions** | Window C → sign in as the scheme's officer (`agri@demo.in` for the subsidy) → **Review inbox** → open the case → type remarks → **Sanction benefit** | The case shows only the department's own evidence. Sanction issues the order in the department system and instructs the treasury; stage 4 completes within seconds and the citizen's page flips to *Completed* with the officer of record and remarks. Show that `official@demo.in` (Skill Development) cannot even see this case. |
| 3:00 | **Same transaction, operator view** | Window A → **Transactions** → paste the TXN in *Search application, transaction or citizen…* → open it | Header shows APP ID ↔ TXN ID. In the **Service journey** tab select each stage: the *Schema transformation* panel shows the **Department response** next to the **Canonical model** with the lineage rows (`SUCCESS → ELIGIBLE`, `DD/MM/YYYY ↔ ISO · transient`, `/payment/state → DISBURSED → COMPLETED`). |
...
| `rohan@demo.in` | Citizen — Rohan Shah | Ownership isolation; his simulated DigiLocker has **no income certificate**, so his seeded scholarship is paused at *Documents required* |
...
cd backend && python -m pytest tests/test_digilocker.py      # 15 checks: DigiLocker consent, pause on missing document, resume, masked preview
cd backend && python -m pytest tests/test_multi_scheme.py tests/test_review_flow.py tests/test_department_routing.py
```
